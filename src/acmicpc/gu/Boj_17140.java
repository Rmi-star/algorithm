...
public class Boj_17140 {
      ...
      public static void main(String[] args) throws Exception {
            ...
            r = Integer.parseInt(st.nextToken()); // k의 위치
      ...
      static int solution() {
            for (int time = 0; time <= 100; time++) { // 100까지 포함하여 계산
                  if (A[r][c] == k) {
                        return time;
                  }
                  operating();
      ...
      // 정렬 : 등장횟수 커지는 순 -> 수가 커지는 순 (우선순위 큐 사용)
      static void operating() {
            if (xLength >= yLength) { // 행 >= 열 (→)
      ...
      static void R(int key) {
            ...
            for (int i = 1; i <= yLength; i++) {
                  ...
                  map.compute(A[key][i], (num, count) -> count == null ? 1 : count + 1); // 기억하기
            }
